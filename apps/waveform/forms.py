    ...
    def clean_vcd_file(self):
        ...
        try:
            ...
            if not markers_found:
                ...
                raise ValidationError('Invalid VCD file format - missing required header section')
                
            if not header.startswith('$') or ('$date' not in header and '$version' not in header):
                logger.warning(f"VCD header check failed for file: {file.name}")
                raise ValidationError('Invalid VCD file format - the file does not appear to be a VCD file')
