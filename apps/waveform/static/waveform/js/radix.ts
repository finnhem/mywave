...
function binToSignedDec(value: string): string {
  ...
  if (!isNegative) {
    ...
    return Number.parseInt(binStr, 2).toString();
  }
  ...
  const absValue = Number.parseInt(inverted, 2) + 1;
 ...
 */
export function formatSignalValue(value: string, signalName: string, forceFormat = false): string {
  ...
  if (signalName) {
    ...
    if (canvas?.signal?.width) {
      ...
      for (const pattern of patterns) {
        ...
        if (match?.[1]) {
          bitWidth = Number.parseInt(match[1], 10) + 1;
          break;
        }
      }
