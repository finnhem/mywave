...
import { startDrag, updateDrag, endDrag, drawOverlay, type DragUpdate, type DragResult } from './canvas';
 ...
 */
export function calculateMaxZoom(minTimeDelta: number, canvasWidth: number, totalTimeRange: number): number {
...
export function handleWheelZoom(event: WheelEvent): void {
    ...
    const timeAtMouse = viewport.visibleStartTime + (mouseX / rect.width) * (viewport.visibleEndTime - viewport.visibleStartTime);
 ...
 */
export function handleZoomDragEnd(event: MouseEvent): void {
